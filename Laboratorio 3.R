...
<li> The longest streak of baskets is of length 4. </li>
  <li> The shortest streak is of length 1.</li> #Este mero
  </ol>
...
sample(coin_outcomes, size = 100, replace = T)
#Este código guarda un vector en coin_outcomes, luego usa la función sample() para generar
#un valor aleatorio de un conjunto de datos; nuestro vector. Luego size le indica el tamaño
#del elementos del vector a seleccionar, si pusieramos dos daría heads y tails a la vez, si pusiéramos 100
#arrojaría 100! Finalment replace hace que la probabilidad sea independiente entre uno y otro evento.
```
...
```{r sim-unfair-coin}
#Con los siguientes ajustes ya no obtenemos un 50/50, sin que la moneda se vuelve cargada
#Prob toma otro vector y aplica las probabilidades de dicho vector a nuestros datos, pr eso la prob()
# es de 20 para head y 80 para tails
...
```{r sim-basket}
#Este es como el código de arriba de la moneda justa pero ahora nuestro vector contiene HIT y MISS
#para simular un jugador de basketball que no puede tener una hot-hand
shot_outcomes <- c("H", "M")
...
<div id="exercise">
  **Exercise**: What change needs to be made to the `sample` function so that it reflects a shooting percentage 
of 45%? Make this adjustment, then run a simulation to sample 133 shots. Assign the output of this simulation 
to a new object called `sim_basket`.
</div>
  ```{r}
# type your code for the Exercise here, and Knit
#Si queremos hacer un jugador similar a los valores de Kobe, entonces hay que hacer que su
#probabilidad de HIT sea de 45%, y que tenga 133 tiros. Esto es bueno, porque sabemos que 
#no puede tener este objeto una hot-hand y será comparable con Kobe
shot_outcomes <- c("H", "M")
...
<div id="exercise">
  **Exercise**: Make a plot of the distribution of simulated streak lengths of the independent shooter.
...
<li> Somewhat similar </li> #Esta mera
...
#ESTE MERO  <li> The distributions look very different. Therefore, there doesn't appear to be evidence for Kobe Bryant's hot hand. </li>
