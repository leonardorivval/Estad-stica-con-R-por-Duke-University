...
<ol>
<li> 2 </li>
<li> 3 </li> # Respuesta1: Esta es la correcta
<li> 4 </li>
<li> 82 </li>
<li> 1710 </li>
</ol>
...
variable in the Data Viewer to answer this question. #Respuesta2: 1929-1710
</div>
...
Let s start to examine the data a little more closely. We can access the data in
         ...
         ```{r view-boys}
         arbuthnot$boys # $ accede a la columna boys del data frame, esto en modo de vector
         ```
...
variable that comes after me".
         ...
         2. What command would you use to extract just the counts of girls born? 
           ...
           <li> `arbuthnot$girls` </li> #Respuesta 3, esta mera
           <li> `girls` </li>
         ...
         side of the printout to indicate locations within the vector. For example, in the arbuthnot$boys vector, 
         5218  follows [1], indicating that 5218 is the first entry in the vector. And if [43] starts a line, 
         then that would mean the first number on that line would represent the 43rd entry in the vector.
         ...
         Before we review the code for this plot, lets summarize the trends we see in the 
data.
...
<li> There is initially an increase in the number of girls baptised, which peaks around 1640. After 1640 there is a decrease 
in the number of girls baptised, but the number begins to increase again in 1660. Overall the trend is an increase 
in the number of girls baptised. </li> #Respuesta 4, esta mera
<li> There is initially an increase in the number of girls baptised. This number peaks around 1640 and then 
after 1640 the number of girls baptised decreases. </li>
...
followed by the name of the function that you re interested in. Try the following in
         ...
         X <-   arbuthnot$boys + arbuthnot$girls
         rm(X)
         ```
         ...
         arbuthnot <- arbuthnot %>%  # %>% permite que output de esta línea de código se una con la siguiente
           mutate(total = boys + girls) #Por medio de mutate x=y se agregan variables
         head(arbuthnot)
         ?mutate()
         arbuthnot <- arbuthnot %>%
           mutate(total = NULL) #Por medio de mutate x= NULL se eliminan variables
         head(arbuthnot)
         ```
...
ggplot(data = arbuthnot, aes(x = year, y = total)) +
  geom_line() #geom line para generar un gráfico de tipo línea
```
...
decreased over time. 
<ol>
<li> True </li> #La respuesta es cierta pero muy leve
<li> False </li>
</ol>
...
present <- present %>%
  mutate(boys_proportion = boys / total)
x <- ggplot(data= present)
x + aes(x =year, y= boys_proportion) +
  geom_point() +
  geom_line()
```
...
<li> Every year there are more boys born than girls. </li> # la respuesta
...
in the `present` dataset. Plot these values over time. Which of the following best describes the trend? 
<ol>
...
<li> There is initially a decrease in the boy-to-girl ratio, and then an increase between 1960 and 1970, followed by a decrease. </li> #Esta es la buena
...
ggplot(data=present, aes(x=year, y=prop_boy_girl <- boys/girls)) +
  geom_line()+
  geom_point()
```
...
<li> 1961 </li>
<li> 1991 </li>
<li> 2007 </li> # esta el la respuesta
</ol>
