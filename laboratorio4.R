...
This data is a modified version of data from the New Zealand Election Survey, deliberately modified to introduce problems that 
occur naturally in many data sets.

## Step One. Learn something about the data set.

In this case, the New Zealand Election Survey takes place every three years as a postal survey of a sample of registered 
electors. Some sampled electors were part of a sample panel of people surveyed at the previous election as part of a 
longitudinal study, others were randomly chosen from the electoral roll. Those electors that were part of the longitudinal panel 
group were randomly selected in previous elections. 

As well as survey results, the data set includes information from the electoral roll, and weighting values for adjusting 
results. The full NZES data set has been reduced to a selected group of variables, making 3101 observations of 107 variables.
...
For example, we might decide that since New Zealand is a Mixed Member Proportional voting system, where people 
get to vote for both an electorate (local) representative and a nationwide political party, that it would be 
interesting to look at strategic voting under conditions where there are many political parties to choose from. 
We identify some relevant variables of interest in the data, and investigate the nature of the individual variables 
before we explore their interactions. The kind of variables they are is going to shape our question. 

### Read in the data

There are many different kinds of data files in the world. Each one has its own issues when being read in by R. 
In this case the data is saved as a .RData file, which can be read in by using the `load()` command.

As with most reading in file commands, inside the parentheses needs to go a piece of text, in quotes, that is the 
path to the file from the working directory (the working directory is the folder that R is currently paying attention to). 
The easiest way to get a R Markdown (Rmd) Document and console cooperating about this is to place the file with the data in 
it in the same folder as the R Markdown (Rmd) Document, open the R Markdown (Rmd) document in RStudio so we are looking at the
contents of the document in the editing window, then in the RStudio Session menu, use the __Set Working Directory - To Source 
File Location__ command to make a common starting point. Then the code in the R Markdown (Rmd) document will use the same working 
space regardless of whether we knit the document or run code chunks in the Console. In this case, if the `nzes2011.RData` file 
is in the same folder as the R Markdown (Rmd) document, hence it can be read into R with the following command:
  
  ```{r get_the_data}
#load permite cargar archivos, y por medio de file.choose() te ahorra en poner la dirección y lo puedes poner direct de tus archivos
load(file.choose())
```
...
As a first question, we might be interested in exploring the relationship between the party the person voted for, 
the party that was their favourite, and if they believed that their vote makes a difference -- focusing on the 
question that are people who believe their vote makes a difference more likely to strategically vote for a party 
not their favourite. To achieve this, we familiarise ourselves with the variables `jpartyvote`, `jdiffvoting`, and 
`_singlefav`. First we check the codebook (see Appendix), then we explore the data.

Viewing the entire dataset in the Data Viewer window by clicking on the data frames name in the Environment or 
running the `View()` command in the Console can be ineffective since the Data Viewer only shows the first 100 
columns of the data frame. 

Using the `str()` command on the entire dataset can also be equally ineffective. However we can subset the columns 
of interest and take a closer look at them. We can use the `dplyr` chain to select the variables of interest and 
investigate only their structure by adding `str()` at the end of the chain:

```{r eval=FALSE}
?select()
#Select literalmente pide un data frame, y los objetos de este que queremos. Por pipping se le da el data frame
#Y se le pide la estructura de estos, aunque no funcionará como veremos por los cambios de nombre a los objetos
selected_nzes2011 %>% 
...
We next need to diagnose where the problem lies -- in the R code or in the data? The best way to troubleshoot this issue 
is to run each line of the `dplyr` chain one by one.
...
We know that `select()` is a valid `dplyr` function, so that cannot be the problem. This means 
the problem might be the variable names. The issue is that R has rules about what variable names are legal 
(e.g. no spaces, starting with a letter) and when data is loaded, R will often fix variable names to make them legal. 
...
We can search the names for a fragment of the name by using the `grep("FRAGMENT", variable, value = TRUE)` 
command, which in this case might be:

```{r findname}
?grep()
#grep() nos ayuda a ver como R puede haber cambiado el nombramiento ilegal de una variable en el data frame
#names indica que queremos saber los nombres del data frame, y value = TRUE que queremos el verdadero
...
An easy way of tabulating these data to see how many times each level of is to use the `group_by()` function along with the 
`summarise()` command:

```{r count_votes}
#Este código ayuda a poder observar los datos agrupados por la variabe jpartyvte
#Y que a esta le saquemos la cantidad de cada caso gracias a summarise() y count= n(),
#lo cual pide el número de cada uno. Si sólo fuera summarise solamente daría los casos y no la frecuencia
selected_nzes2011 %>% 
  group_by(jpartyvote) %>% 
  summarise(count = n())
```

We can see that 23 people answered `"Don't know"`. Since our question is about people who knew which party they voted for, 
we might want to exclude these observations from our analysis. We can do so by `filter`ing them out.

```{r only_certainty}
?filter()
#Este código usa filter() para que los datos del data frame que cumplen 
#con la condición sean mantenidos, en este caso todos los que no son "Dont't know"-
#Luego se despliegan nuevamente los datos por group_by() y summarise(count= n())
selected_nzes2011 %>% 
...
Because there is a `%>%` at the end of the line, R knows to continue on to the next line, as with any other 
'to be continued'
symbol at the end of the line.

Note that adding the filter also got rid of the `NA` entries. NA (Not Available) is used to indicate blank entries -- 
those observations for which there is no data recorded. It is always a good plan to be aware of NAs and deliberately include 
them in or exclude them from the analysis so that the final results are not surprising. In this case since NA indicates that 
these people did not answer the question about which party they voted for, exluding them from the analysis makes sense.
...
This set also has `NA` entries, but in this case we don t want to get rid of anything but the `NA`s so we need to target them 
directly. `NA` entries need special targeting because they do not actually exist (they are different to the text `"NA"` or 
a variable saved with the name `NA`). 

If we only wanted to find the `NA`s we would use the `is.na()` function with the name of the variable inside the parentheses. 

However since we want the entries that are **not** `NA`s we can use the __Not__ operator, `!`, to indicate 
"we want all the ones that are not NA":`!is.na()`. Hence we can `filter` out all non NAs in our `dplyr` chain:
  
  ```{r count_known_favourites}
#Este código filtra los datos por los que no son NA gracias a la función is.na más el símbolo lógico !
selected_nzes2011 %>% 
...
#Si jusntamos nuestros dos códigos de arriba, el que filtra por !is.na y el que lo hace por !=, obtenemos el mismo resultado solamente que de un jalón
#La diferencia es que no se mostrará el de !=
selected_nzes2011 %>% 
...
Remember that our main question is about whether people vote for their favorite party or a diffent one. Hence an 
straighforwrd approach would be to first determine whether each observation in the data represents a person who voted 
for the party same as their favorite party or different. This requires creating a new variable with the `mutate()` function.

In creating this variable we want to evaluate if for a given observation the values in the `jpartyvote` 
and `X_singlefav` variables are the same, or different:
  
  ```{r make_sameparty}
#Este código lo qu hace es generar una nueva variable llmada sameparty a partir de la función ifelse().
#Si jpartyvote (el partido por el cual votaron) es igual a X_singlefav (su partido favorito), entonces se guarda como "same,
#si no se guarda como "different"
...
We can again check our work by exploring the groupings in a View:
  ...
  ```{r view_sameparty}
#Ahora mediante este código podemos tomar el data fram, agrupar por las variables deseadas, y luego sacar las cuentas dadas esass variables
selected_nzes2011 %>% 
...
We can see that observations where `jpartyvote` equaled `X_singlefav`, the value `"same"` was recorded for the 
new variable `sameparty`, and the value `"different"` was recorded otherwise. If either `jpartyvote` or `X_singlefav` 
had an `NA`, R could not check for equality and hence `NA` was recorded for the `sameparty` variable as well.

To view and summarize the "same" entries we can use the following:
  
  ```{r filter_keep_same}
#Este código permite ver los resúmenes pero con los datos de same solamente
selected_nzes2011 %>% 
  group_by(jpartyvote, X_singlefav, sameparty) %>%
  summarise(count = n()) %>% 
...
#Lo mismo que el anterior pero con different
selected_nzes2011 %>% 
  group_by(jpartyvote, X_singlefav, sameparty) %>%
  summarise(count = n()) %>% 
...
We can also check how we got any `NA`s we have by using the `is.na()` function:
  ...
  ```{r filter_keep_na}
#Lo mismo que los dos anteriores per ahora tenemos que lo filtra por los que son NA
#Ya sea cuando en jparty o x_singlefav tengan NA sameparty simpre va a ser NA si uno lo es 
selected_nzes2011 %>% 
  group_by(jpartyvote, X_singlefav, sameparty) %>%
  summarise(count = n()) %>% 
...
The checks show that the observations with `NA`s in the `sameparty`are going to be excluded from the analysis when we 
fiter out the `NA`s in the `jpartyvote` and `X_singlefav` variables, so we don t need to worry about them anymore. 
...
and how much they like the NZ First party. We become familiar with the variables `jnzflike` and `jage` in the codebook, 
then explore the data.

```{r str_q2}
#Para saber la estructura de  dichas variabales jnzlikke (que tanto les gusta el partido nz) y jage(la edad del votante)
str(selected_nzes2011$jnzflike)
str(selected_nzes2011$jage)
```

`jnzflike` is a factor variable, in fact it is ordinal and by default the levels are listed in alphabetical order. 
Since this is a categorical variable, we can also summarize the occurances of each level with `group_by()` and `summarise()` 
again:
  
  ```{r factor_counts}
#Como jnzlike es una variable de tipo factor podemos hacer uso de summrise y group_by para saber las frcuencias
selected_nzes2011 %>% 
  group_by(jnzflike) %>% 
  summarise(count = n())
```
...
`jage`, on the other hand, is an integer, with values that are whole numbers between 0 and infinity (or `NA`).
For this variable we would want to take a look at numerical summaries such as means, medians, etc.

```{r numeric_summary}
#Por otro lado jage es un entero así que podemos thacer uso de la media, la mediana, la sd, máx y mín.
#Sin embargo no funciona! Esto es porque NAs no son enteros y deben ser excluídos
selected_nzes2011 %>% 
  summarise(agemean = mean(jage), agemedian = median(jage), agesd = sd(jage), 
            agemin = min(jage), agemax = max(jage))
```

What went wrong? The reason why all of the results were reported as NAs is that there were some NA entries in the `jage` 
variable (people not reporting their age). Since it is not possible to take the average of a series of values that contain 
`NA`s, obtaining the numerical summaries requires that we exclude the `NA`s from the calculation.

Most numerical summary functions allow us to easily exclude `NA`s with the `na.rm` argument. See the help documentation for the 
`median` function for more information.

```{r see_help}
#Podmeos ver que mediant na.rm estos se eliminan antes de hacer el cálculo
?median
```

An alternative approach is just to `filter` out the `NA`s first, 
and then ask for the numerical summaries:
  
  ```{r nonna_numeric_summaries}
#Otra opción es usar la función filter() y !is.na(), y luego sacar las medidas de tendencia central
selected_nzes2011 %>% 
  filter(!is.na(jage)) %>%
  summarise(agemean = mean(jage), agemedian = median(jage), agesd = sd(jage), 
            agemin = min(jage), agemax = max(jage))
...
we next need to consider if there is additional work we should do on the data 
in investigating the question. There are a number of different approaches we 
might take. For example, we could consider if those that strongly like NZ 
First are older than those that strongly dislike NZ First, or we could consider 
if old people like NZ First more than young people.
...
come in handy:
  ...
  ```{r pick_from_list}
#El código hace que los objetos de la variable se filtren dado el vector c("0", "10")
#Luego a partir de esto los agrupa y saca sus frecuencias, En este caso nos da el número de los valores más extremos de jnzflike
selected_nzes2011 %>% 
