...
The `nycflights` data frame is a massive trove of information. Lets think about 
...
</div> #Est·n poco claros ambos, quiz· est· mejor el de 15, pero a˙n asÌ el sesgo a la izquierda no ayuda
...
```{r rdu-flights-hist}
rdu_flights <- nycflights %>% #Este bonito cÛdigo filtra los destinos para tener solo los de RDU, esto mediante ==
  filter(dest == "RDU")       #Esto almacenando en rdu_flights los datos correspondientes solamente a RDU
ggplot(data = rdu_flights, aes(x = dep_delay)) + #Esto plotea el histograma de los datos de RDU
  geom_histogram()
```
...
#Una prueba con varias de las funciones que puede alojar summarise()
rdu_flights %>%
  summarise(mean_dd = mean(dep_delay), sd_dd = sd(dep_delay), min_dd_ =min(dep_delay),
            max_dd = max(dep_delay), IQR_dd = IQR(dep_delay), var_dd= var(dep_delay),
            median_dd= median(dep_delay), n = n())
...
```{r}
sfo_feb_flights <- nycflights %>%   #Esto filtra en funciÛn de dos condiciones ==
  filter(dest == "SFO", month == 2) #Si se quisiera usar |, entonces con una sola condiciÛn bastarÌa y no ambas
```
...
<ol>
<li> 68 </li>  #Esta mera es la buena
<li> 1345 </li> 
<li> 2286 </li> 
<li> 3563 </li>
<li> 32735 </li>
...
<li> No flight is delayed more than 2 hours. </li>  #Esta mera es la buena
...
rdu_flights %>%
  group_by(origin) %>% #Esto agrupa los datos en funciÛn de la variable origen
  summarise(mean_dd = mean(dep_delay), sd_dd = sd(dep_delay), n = n()) #Luego esto hace los res˙menes estadÌsticos del retraso para cada origen
```
...
<li> Delta and United Airlines </li> #Esta mera es la buena
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(IQR_sfo= IQR(arr_delay))
...
Let s think about how we would answer this question:
                               ...
                               <li> Median would be more reliable as the distribution of delays is skewed. </li>  #Esta mera es la buena
                                             ...
                                             side-by-side box plots:
                                               ...
                                               ```{r delay-month-box}
                                             ggplot(nycflights, aes(x = factor(month), y = dep_delay)) + #Factor() fuerza una vriable a ser tratada como factor
                                               geom_boxplot()  #Est es necesario debido a que el boxplot necesita una categorÌa en el eje x para hacer el ploteo lado a lado
                                             ```
...
#Este cÛdigo genera por ifelse() una prueba que si se cumple asigna "on time", y si no "delayed"
...
#Ese cÛdigo primero toma el data frame de nycflight, luego nos agrupa los datos por
#el origen, posteriormente hace un resumen sacando la proporciÛn de los vuelos a tiempo,
#y finalmente los acomoda de mayor a menor.
```
...
<li> EWR </li> 
<li> JFK </li> 
<li> LGA </li> #Este mero es el bueno
</ol>
...
ggplot(data = nycflights, aes(x = origin, fill = dep_type)) +
  geom_bar()
#Literalmente estamos ploteando una tabla de contingencia por medio de un gr·fico de barras
...
<li> There is an overall postive association between distance and average speed.</li> #Esta mera
...
ggplot(data= nycflights, aes(x=avg_speed<-distance/(air_time/ 60), y=distance))+
  geom_point()
head(nycflights)
```
...
nycflights <- nycflights %>%
  mutate(arr_type = ifelse(arr_delay <= 0, "on_time", "delay")) 

nycflights <- nycflights %>%
  mutate(dep_type2 = ifelse(dep_delay <= 0, "on_time", "delayed"))

nycflights%>%
  summarise(prop = sum(arr_type == "on_time" & dep_type2 == "delayed")/ sum(dep_type2 == "delayed"))
